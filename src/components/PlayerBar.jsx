...
export default function PlayerBar({ currentSong, isPlaying, shuffle, repeatMode, currentTime, duration, volume, muted, onTogglePlay, onPrev, onNext, onToggleShuffle, onToggleRepeat, onSeek, onSetVolume, onToggleMute }) {
  ...
  return <footer className="player-bar">
    ...
    <div className="player-center"><div className="controls"><button className={`ctrl ${shuffle ? 'on' : ''}`} onClick={onToggleShuffle} title="Shuffle" aria-label="Shuffle">⤨</button><button className="ctrl" onClick={onPrev} title="Previous" aria-label="Previous song">◀◀</button><button className="play-pause" onClick={onTogglePlay} aria-label={isPlaying ? 'Pause' : 'Play'}>{isPlaying ? 'Ⅱ' : '▶'}</button><button className="ctrl" onClick={onNext} title="Next" aria-label="Next song">▶▶</button><button className={`ctrl ${repeatMode !== 'off' ? 'on' : ''}`} onClick={onToggleRepeat} title={`Repeat ${repeatMode}`} aria-label="Repeat">↻</button></div><div className="progress-row"><span className="time">{fmt(currentTime)}</span><div className="progress-track" onClick={seekFromEvent}><div className="progress-fill" style={{ width: `${duration ? currentTime / duration * 100 : 0}%` }} /></div><span className="time">{fmt(duration)}</span></div></div>
